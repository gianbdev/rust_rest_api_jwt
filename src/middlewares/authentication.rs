use crate::{config::db::Pool, config::routes, jwt::UserToken, models::user::User};
    ...
    B: 'static,
{
    ...
    fn call(&mut self, mut request: ServiceRequest) -> Self::Future {
        ...
        let str_authen_header = match authen_header.to_str() {
            ...
            Err(_) => {
                return Box::pin(async move {
                    Ok(request.into_response(
                        HttpResponse::Unauthorized()
                            .body("The authorization header doesn't seem to be stringifyable")
                            .into_body(),
                    ))
                });
            }
        ...
        if !str_authen_header.starts_with("Bearer") && !str_authen_header.starts_with("bearer") {
            return Box::pin(async move {
                Ok(request.into_response(
                    HttpResponse::Unauthorized()
